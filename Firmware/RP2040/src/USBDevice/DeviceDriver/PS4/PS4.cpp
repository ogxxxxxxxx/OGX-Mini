...
void PS4Device::process(const uint8_t idx, Gamepad& gamepad)
{
    ...
    // ============ ENVIAR REPORT 0x01 (64 BYTES) COMO GP2040-CE ============
    if (tud_hid_ready())
    {
        tud_hid_report(
            0, 
                                  ...
                                  uint8_t *buffer, uint16_t reqlen)
{
    ...
    if (report_type == HID_REPORT_TYPE_FEATURE)
    {
        ...
        else if (report_id == 0xA3)
        {
            ...
            memcpy(buffer, output_0xa3, len);
            return len;
        }
        // ============ REPORT 0xF1 (WARZONE AUTH - COMO GP2040-CE) ============
        else if (report_id == 0xF1)
        {
            printf("[PS4] -> Enviando Report 0xF1 chunk %d\n", cur_nonce_chunk_);
            
            uint8_t data[64];
            data[0] = 0xF1;
            data[1] = 1;  // nonce_id
            data[2] = cur_nonce_chunk_;
            data[3] = 0;
            
            // Copiar 56 bytes del buffer de autenticación
            memcpy(&data[4], &report_0x11_.data[cur_nonce_chunk_ * 56], 56);
            
            // CRC32
            uint32_t crc = calculate_ds4_crc32(data, 60);
            memcpy(&data[60], &crc, sizeof(uint32_t));
            
            memcpy(buffer, &data[1], 63);
            
            cur_nonce_chunk_++;
            if (cur_nonce_chunk_ == 19) {
                cur_nonce_chunk_ = 0;
            }
