...
void XInputDevice::process(const uint8_t idx, Gamepad& gamepad)
{
    ...
    if (gamepad.new_pad_in())
    {
        ...
        // Remapped: MUTE (SYS) -> SELECT (BACK)
        if (gp_in.buttons & Gamepad::BUTTON_SYS)   in_report_.buttons[0] |= XInput::Buttons0::BACK;

        // L1 (LB) -> Turbo Antirecoil for X
        bool current_lb = gp_in.buttons & Gamepad::BUTTON_LB;
        if (current_lb) {
            turbo_toggle++;
            if (turbo_toggle % 2 == 0) {  // Toggle X for turbo
